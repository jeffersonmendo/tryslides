  ...
  return (
    ...
    >
      ...
      >
        <button
          ref={handleRef}
          aria-label={`${element.type}. ${t("moveInstruction")}${isReference ? `. ${t("referenceAlignmentSelected")}` : ""}`}
          ...
          onKeyDown={(event) => {
            ...
            if (!isSelected || event.metaKey || event.ctrlKey)
              onSelect(element.id, event.metaKey || event.ctrlKey);
          }}
          onPointerDown={(event) => {
            if (event.shiftKey && isSelected && canSetReference) {
          ...
          <div className="pointer-events-none absolute inset-0">
            ...
            {canResizeElement(element.type)
              ? RESIZE_HANDLES.filter(
                  ...
                  return (
                    <button
                      aria-label={`${t("resizeElement")}: ${t(getResizeHandleKey(handle.direction))}`}
            ...
            <div className="absolute left-1/2 top-0" data-selection-center-axis>
              {north_handle_size !== null ? (
                <button
                  aria-label={`${t("resizeElement")}: ${t("resizeNorth")}`}
              ...
              />
              <button
                aria-description={t("rotationInstruction")}
                aria-label={t("rotationElement")}
...
function getResizeHandleKey(handle: ResizeHandle) {
  return {
    north: "resizeNorth",
    "north-east": "resizeNorthEast",
    east: "resizeEast",
    "south-east": "resizeSouthEast",
    south: "resizeSouth",
    "south-west": "resizeSouthWest",
    west: "resizeWest",
    "north-west": "resizeNorthWest",
  }[handle];
}
