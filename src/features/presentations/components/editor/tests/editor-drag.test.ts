...
test("previews every selected element with the shared, bounds-constrained drag delta", () => {
  const previews = getDragPreviewPositions({
    canvas: CANVAS,
    elements: [
      {
        id: "elementOne",
    ...
    sourceId: "elementOne",
    sourcePosition: { x: 800, y: 300 },
  });

  assert.deepEqual(previews, {
    elementOne: { x: 520, y: 300 },
    elementTwo: { x: 1_720, y: 300 },
  });
});

test("persists every selected element after dragging one selected member", async () => {
  ...
  const elements = createElements(slide.state, {
    slideId: "slideOne",
  ...
  let persisted_state = elements.state;
  const result = await commitDragPreview(
    { elementId: "elementOne", position: { x: 700, y: 500 } },
    ["elementOne", "elementTwo"],
    async (element_id, x, y, element_ids) => {
      const source = persisted_state.slides[0]?.elements.find(
        (element) => element.id === element_id,
      );
      assert.notEqual(source, undefined);
      if (source === undefined) return { persisted: false };
      const moved = moveElements(persisted_state, {
        slideId: "slideOne",
        elementIds: element_ids,
        delta: { x: x - source.position.x, y: y - source.position.y },
        updatedAt: "2026-09-12T12:03:00.000Z",
      });
      assert.equal(moved.success, true);
      if (!moved.success) return { persisted: false };
      persisted_state = moved.state;
      return { persisted: true };
    },
  );
  ...
  assert.deepEqual(
    persisted_state.slides[0]?.elements.map((element) => element.position),
    [
      { x: 700, y: 500 },
      { x: 1100, y: 500 },
    ],
...
test("preserves the initial multi-selection through a completed or no-op drag", () => {
  const source = readFileSync(
    new URL("../slide-renderer.tsx", import.meta.url),
    "utf8",
  );
  const drag_start_source = source.slice(
    source.indexOf("function handleDragStart"),
    source.indexOf("function handleDragMove"),
  );
  ...
  assert.match(
    drag_start_source,
    /drag_element_ids_ref\.current = selectionIds\.includes\(source\.id\)\s*\? selectionIds\s*: \[source\.id\]/,
  );
  assert.match(
    drag_end_source,
    /commitDragPreview\(preview, getDragElementIds\(source\), onMoveEnd\)/,
  );
  assert.doesNotMatch(drag_end_source, /onSelectElement|onDeselectElement/);

  const element_source = readFileSync(
    new URL("../editor-element.tsx", import.meta.url),
    "utf8",
  );
  const pointer_down_source = element_source.slice(
    element_source.indexOf("onPointerDown={(event) => {"),
    element_source.indexOf(
      "        >\n          <div",
      element_source.indexOf("onPointerDown={(event) => {"),
    ),
  );
  ...
  assert.match(
    pointer_down_source,
    /if \(!isSelected \|\| event\.metaKey \|\| event\.ctrlKey\)\s*onSelect\(element\.id, event\.metaKey \|\| event\.ctrlKey\)/,
  );
  assert.match(
    element_source,
    /onFocus=\{\(\) => \{\s*if \(!isSelected\) onSelect\(element\.id\);\s*\}\}/,
  );
});
...
test("does not invoke the terminal move callback for a zero-displacement drag", async () => {
  ...
  assert.match(drag_end_source, /setDragPreview\(preview, positions\)/);
  assert.ok(
    drag_end_source.indexOf("preview.position.x === source.position.x") <
      drag_end_source.indexOf("setDragPreview(preview, positions)"),
  );
});
...
test("keeps selection chrome inside the single rotated interaction plane", () => {
  ...
  assert.match(
    source,
    /north_handle_size\?\.height \?\? 0[\s\S]*data-selection-center-axis[\s\S]*north_handle_size[\s\S]*rotation_connector[\s\S]*t\("rotationElement"\)/,
  );
});
...
test("synchronously presents each drag preview before making it available to drag end", () => {
  ...
  assert.match(
    source,
    /handleDragMove[\s\S]*getDragPreviewPositions[\s\S]*setDragPreview\(\{ elementId: source\.id, position \}, positions\)/,
  );
  ...
  assert.match(
    source,
    /handleDragEnd[\s\S]*setDragPreview\(preview, positions\)/,
  );
