...
export function getDragPreviewPositions({
  canvas,
  elements,
  sourceId,
  sourcePosition,
  ...
  readonly elements: readonly DraggableElement[];
  ...
  const delta = getClampedMovementDelta(
    elements,
    {
...
function getClampedMovementDelta(
  elements: readonly DraggableElement[],
  requested: Position,
  canvas: Canvas,
): Position {
  ...
  for (const element of elements) {
    ...
    maximum_x = Math.min(
      maximum_x,
      canvas.width - element.size.width / 2 - element.position.x,
    );
    minimum_y = Math.max(
      minimum_y,
    ...
    maximum_y = Math.min(
      maximum_y,
      canvas.height - element.size.height / 2 - element.position.y,
    );
  }
  return {
    x: clamp(requested.x, minimum_x, maximum_x),
    y: clamp(requested.y, minimum_y, maximum_y),
  };
}
