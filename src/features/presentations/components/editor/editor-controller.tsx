...
import type { EditorIntentDraft } from "./lib/editor-drafts";
import type { DragCommitResult } from "./lib/editor-drag";
import {
  hasGeometryPatch,
  normalizeGeometryDraftPatch,
} from "./lib/editor-geometry-draft";
import {
  getImageUrlRecordIfChanged,
  type ImageAssetReference,
  type ImageUrlEntry,
  reconcileImageUrlEntries,
} from "./lib/editor-image-urls";
import type { EditorSelection } from "./lib/editor-model";
import { toEditorSlide } from "./lib/editor-projection";
import { useEditorSession } from "./lib/use-editor-session";
...
export function EditorController({
  ...
  function scheduleElementsRotation(
    slide_id: string,
    element_ids: readonly string[],
    delta: number,
  ): string {
    const normalized_ids = [...element_ids].sort();
    const key = `rotation:${slide_id}:${normalized_ids.join(",")}`;
    scheduler_ref.current?.schedule({
      key,
      draft: {
        kind: "rotation",
  ...
  function scheduleElementsOpacity(
    slide_id: string,
    element_ids: readonly string[],
    opacity: number,
  ): string {
    const normalized_ids = [...element_ids].sort();
    const key = `opacity:${slide_id}:${normalized_ids.join(",")}`;
    scheduler_ref.current?.schedule({
      key,
      draft: {
        kind: "opacity",
  ...
  return (
    <EditorShell
      ...
      onMoveEnd={async (element_id, x, y, dragged_element_ids) => {
        ...
        return runCommand((state) => {
          ...
          const element = elements.find((current) => current.id === element_id);
          const element_ids = dragged_element_ids.filter((id) =>
            elements.some((current) => current.id === id),
          );
          return element_ids.length > 1 &&
      ...
      onRotateElementsChange={(element_ids, delta) => {
        if (active_slide_id === null) return;
        scheduleElementsRotation(active_slide_id, element_ids, delta);
      }}
      onRotateElementsCommit={(element_ids, delta) => {
        if (active_slide_id === null) return;
        const key = scheduleElementsRotation(
          active_slide_id,
          element_ids,
          delta,
        );
      ...
      onSetElementsOpacityChange={(element_ids, opacity) => {
        if (active_slide_id === null) return;
        scheduleElementsOpacity(active_slide_id, element_ids, opacity);
      }}
      onSetElementsOpacityCommit={(element_ids, opacity) => {
        if (active_slide_id === null) return;
        const key = scheduleElementsOpacity(
          active_slide_id,
          element_ids,
          opacity,
        );
